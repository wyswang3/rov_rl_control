...
当前支持的轨迹类型：
  ...
  - constant_acc    : 常加速度（扣重力后保持 [1,1,1]）
"""
...
def linear_path(start: np.ndarray, end: np.ndarray, steps: int = STEPS) -> np.ndarray:
    ...
    return np.linspace(start, end, steps, axis=0).astype(np.float32)
...
def generate_feasible_trajectory(target: Dict) -> List[Dict]:
    ...
    elif ttype == "constant_acc":
        return gen_constant_acc_trajectory(target)
    else:
        return gen_hover_trajectory(target)
...
def gen_constant_acc_trajectory(target: Dict) -> List[Dict]:
    """
    常加速度轨迹：在整个 T 内保持线性加速度 a_const（不含重力），
    角速度固定为 0，姿态保持不变（四元数恒定）。
    target 中应包含:
      - "acc": List[float]  形如 [ax, ay, az]，线加速度（不含重力）
      - "quat": List[float] 四元数 [w, x, y, z]（默认 [1,0,0,0]）
    """
    a_const = np.asarray(target.get("acc", [0.0, 0.0, 0.0]), dtype=np.float32)
    quat0   = np.asarray(target.get("quat", [1.0, 0.0, 0.0, 0.0]), dtype=np.float32)

    trajectory: List[Dict] = []
    vel = np.zeros(3, dtype=np.float32)
    pos = np.zeros(3, dtype=np.float32)
    ang_vel = np.zeros(3, dtype=np.float32)   # 始终为 0
    quat_fixed = quat0.copy()                # 姿态固定不变

    for i in range(STEPS):
        trajectory.append({
            "pos": pos.copy(),
            "quat": quat_fixed.copy(),
            "vel": vel.copy(),
...
def compute_physical_acceleration(trajectory: List[Dict], dt: float = DT) -> np.ndarray:
    ...
    lin_acc = np.gradient(vel_seq, dt, axis=0).astype(np.float32)   # (steps,3)
    ang_acc = np.gradient(angvel_seq, dt, axis=0).astype(np.float32) # (steps,3)

    return np.hstack([lin_acc, ang_acc])
